...
/* ════════════════════════════════════════════════════════════════════════════
   CLOUD AUTH — Facade over Supabase (prod) or localStorage mock (localhost).

   Auth + profile + ramp presets only. Game APIs (runs, beta, leaderboard) live
   in the app as assets/js/cloud-game.js and Object.assign onto window.Cloud.

   Script load order (index.html / stats.html): profile-validation.js, backend
   script(s) load async via loadCloudScripts(), then auth.js (+ cloud-game.js);
   page scripts (stats.js, roguelite.js) may run before getSession() finishes.
   Consumers MUST use Cloud.onAuth(fn) — never assume auth is resolved on first tick.
   ...
   Public API (window.Cloud):
     ...
     getClient()                    — backend Supabase client or null (apps query own tables)
     getBackend()                   — advanced: raw backend module
   ════════════════════════════════════════════════════════════════════════════ */
const Cloud = (() => {
  const listeners = [];
  let backend = null;
  ...
  function onBackendAuth(user) {
    ...
    // before calling setUser/onAuthChange (see supabase-backend.js getSession).
  ...
  function getClient() {
    if (!init()) return null;
    return backend && backend.getClient ? backend.getClient() : null;
  }
