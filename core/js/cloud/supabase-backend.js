...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   Loaded only on production hosts; wired by js/cloud/auth.js facade.

   Auth init contract: isReady() must be true BEFORE the first onAuthChange call
   (auth.js reads isReady() inside onBackendAuth). getSession() is async — set
   ready=true then setUser(), matching the sync order in mock-backend.js init().
   ════════════════════════════════════════════════════════════════════════════ */
const CloudSupabaseBackend = (() => {
  ...
  return {
    init, isReady, signIn, signOut, currentUser, getSessionUser, getClient,
